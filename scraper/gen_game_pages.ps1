...
function Build-National($key) {
  $n = $NAT[$key]; $g = $data.games.$key; if (-not $g) { return 0 }
  $label = $g.label
...
function Build-State($key) {
  ...
  $intro = "$label is a $stateLine" + "lottery game, drawn $draws, with tickets from $price.$oddsLine $analysis Like every lottery game it is a negative-expected-value bet &mdash; played for entertainment, not investment."
